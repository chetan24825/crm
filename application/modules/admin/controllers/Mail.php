...
class Mail extends Admin_Controller
{
    ...
    public function inboxList(){
        ...
        foreach ($list as $item) {
            ...
            if($item->rating) {
                $row[] = '<a onclick="changeStatus(this)" id="' . str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($item->id)) . '" href="javascript:;">
                      <i class="fa fa-star text-yellow"></i></a>';
            }else{
                $row[] = '<a onclick="changeStatus(this)" id="' . str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($item->id)) . '" href="javascript:;">
                      <i class="fa fa-star-o text-yellow"></i></a>';
            }
            $row[] = '<a href="'.base_url('admin/mail/viewEmail').'/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($item->id)).'">'.$item->sender_name.'</a>';
            if($item->reading){
    ...
    function domainEmail($id = null){
        ...
        $this->mViewData['domain'] = $domain;
        $this->render('mail/domain');
    }

    function hostingEmail($id = null){
        $id = $this->encrypt->decode(str_replace(array('-', '_', '~'), array('+', '/', '='), $id));
        ...
        $this->db->set('reading', 1, FALSE)->where('id', $id)->update('customer_hosting'); 

        //customer
        $customer = $this->db->get_where('customer', array(
            'id' => $hosting->customer_id
        ))->row();
        $this->mViewData['customer'] = $customer;
        $this->mViewData['hosting'] = $hosting;
        $this->render('mail/hosting');
    }
    ...
    function viewEmail($id = null)
    {
        ...
        $result == TRUE || $this->message->norecord_found('admin/mail');
    ...
    function forwardMail($id)
    {
       ...
       $mailType = explode("*", $id);
        ...
        $mail == TRUE || $this->message->norecord_found('admin/mail');
    ...
    public function sentBoxList(){
        ...
        foreach ($list as $item) {
            ...
            $row[] = '<a href="'.base_url('admin/mail/viewSentEmail').'/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($item->id)).'">'.'To: '.implode(", ",$to).'</a>';
            $row[] = $item->subject .' - '. substr(strip_tags($item->msg),0,20).'...';
    ...
    function viewSentEmail($id = null)
    {
        ...
        $result == TRUE || $this->message->norecord_found('admin/mail/sentItem');
                ...
                if ($hasAttachmentInbox || $hasAttachmentSentItem) {
                    ...
                    if (count($attachmentFiles)) {
                        foreach ($attachmentFiles as $item) {
                            $file = ATTACHMENT . $item->file_name;
                            unlink($file);
                        }
                    }
                    $this->db->delete($tablePrimary, array('id' => $result[0]));
    ...
    function sendDomainEmail()
    {
        ...
        if($mailType == 'smtp')
        {
            ...
            if ($this->email->send())
            {
                $this->message->custom_success_msg('admin/home', lang('your_email_has_been_send_successfully!'));
            }else{
                $this->message->custom_error_msg('admin/home', lang('fail_to_send_your_email'));
            }
        }
    }

    function sendHostingEmail()
    {
        $mailType = get_option('email_send_option');
        if($mailType == 'smtp')
        {
