...
class Crud_Model  extends MY_Model
{
    ...
    function _callback_action_website($value, $row){
        return '<span class="cust_website">'.$row->website.'</span>';
    }
    
    function _callback_action_estimateDate($value, $row){
        return $this->localization->dateFormat($row->estimate_date);
    }

    function _callback_action_expiredDate($value, $row){
        return $this->localization->dateFormat($row->valid_until);
    }
    ...
    function _callback_action_customer($value, $row){
    ...
    function _callback_action_all_estimate($value, $row){
        $estimateID = strip_tags($row->id);
            return '
                <div class="btn-group dropdown">
                    ...
                    <ul class="dropdown-menu pull-right">                        
                        <li> <a href="estimates/preview/' . $estimateID . '"><i class="fa fa-eye text-success"></i>'. lang('view').'</a> </li>
                        <li> <a href="estimates/updateEstimate/' . $estimateID . '"><i class="fa fa-shopping-cart text-success"></i>'. lang('edit').'</a></li>
                        <li> <a href="estimates/createPdfInvoice/' . $estimateID . '"><i class="fa fa-file-pdf-o text-success"></i>PDF Estimate</a> </li>
                        <li> <a href="estimates/sendEstimate/' . $estimateID . '" data-target="#modalSmall" data-toggle="modal"><i class="fa fa-envelope-o text-success" ></i>Email Estimate</a> </li>
                        <li><a onclick="return confirm(\'Are you sure want to delete this Invoice ?\');" href="estimates/deleteEstimate/' . $estimateID . '"><i class="fa fa-trash-o text-danger"></i>'. lang('delete').'</a> </li>
                    </ul>
                </div>
            ';
    }
    ...
    function _callback_action_estimate_status($value, $row)
    {
        if($row->status === 'draft'){
            return '<span class="label bg-red">Draft</span>';
        }elseif ($row->status === 'accepted'){
            return '<span class="label bg-aqua-active">Accepted</span>';
        }elseif ($row->status === 'declined'){
            return '<span class="label bg-orange">Declined</span>';
        }elseif ($row->status === 'sent'){
            return '<span class="label bg-olive-active">Sent</span>';
        }
    }
