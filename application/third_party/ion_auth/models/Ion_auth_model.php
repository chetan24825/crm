...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->identity_email_column = $this->config->item('identity_email', 'ion_auth');
	 ...
	 * @author Mathew
	 **/
	public function deactivate_customer($id = NULL)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['customer_users'], $data, array('id' => $id));
	 ...
	 * @author Mathew
	 **/
	public function identity_email_check($identity = '')
	{
		$this->trigger_events('identity_email_check');

		if (empty($identity))
		{
			return FALSE;
		}
		return $this->db->where($this->identity_email_column, $identity)
		                ->count_all_results($this->tables['customer_users']) > 0;
	}
	 ...
	 * @author Mathew
	 **/
	public function customer_register($identity, $password, $email, $additional_data = array(), $groups = array())
	{

		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('customer_manual_activation', 'ion_auth');

		// if ($this->identity_email_check($identity))
		// {
		...
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('customer_group', 'ion_auth')),1)->row();
		...
		// Users table.
		$data = array(
		    $this->identity_email_column   => $identity,
		    'password'   => $password,
		    'email'      => $email,
		...
		$user_data = array_merge($this->_filter_data($this->tables['customer_users'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['customer_users'], $user_data);
	 ...
	 * @author Ben Edmunds
	 **/
	public function customer_user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['customer_users'].'.id', 'desc');
		$this->where($this->tables['customer_users'].'.id', $id);
	...
	public function customerusers($groups = NULL)
	{
		$this->trigger_events('users');
		...
		else
		{
			//default selects
			$this->db->select(array(
			    $this->tables['customer_users'].'.*',
			    $this->tables['customer_users'].'.id as id',
			    $this->tables['customer_users'].'.id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    $this->tables['users_groups'],
				    $this->tables['users_groups'].'.'.$this->join['customer_users'].'='.$this->tables['customer_users'].'.id',
				    'inner'
				);
			}
		...
		$this->response = $this->db->get($this->tables['customer_users']);
