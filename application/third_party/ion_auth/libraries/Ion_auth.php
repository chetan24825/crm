...
class Ion_auth
{
	...
	public function customer_register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('customer_email_activation', 'ion_auth');
		
		$id = $this->ion_auth_model->customer_register($identity, $password, $email, $additional_data, $group_ids);
		...
		else
		{
			...
			// deactivate so the user much follow the activation flow
			$deactivate = $this->ion_auth_model->deactivate_customer($id);
			...
			$activation_code = $this->ion_auth_model->activation_code;
			$identity        = $this->config->item('identity_email', 'ion_auth');
			$user            = $this->ion_auth_model->customer_user($id)->row();
			$data = array(
				'identity'   => $user->{$identity},
				'id'         => $user->id,
				'email'      => $email,
			...
			if(!$this->config->item('use_customer_ci_email', 'ion_auth'))
			{
				$this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
				$this->set_message('activation_email_successful');
				return $data;
